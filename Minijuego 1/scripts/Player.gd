...
# ══════════════════════════════════════════════════════════════════
#  Player.gd — v6: Dash + Combo Visual + Touch/Laptop
# ══════════════════════════════════════════════════════════════════

const SPEED: float         = 240.0   # 🏃‍♂️ Subimos de 210 a 240 para que corra con más impulso inicial
const JUMP_VELOCITY: float = -680.0  # 🚀 Subimos de -560 a -620 para que el impulso hacia arriba sea mayor
const GRAVITY: float       = 1200.0  # ⚖️ Ajustamos la gravedad a 1150 para que la caída no sea "flotante" y mantenga el ritmo rápido
...
# 🌟 Partículas de dash (creadas por código)
var particulas_dash: Array = []

# 🌟 Label de combo (se crea dinámico en el HUD)
...
func _ready() -> void:
	...
	# Conectar señal de combo del GameManager
	if GameManager.has_signal("combo_changed"):
		GameManager.combo_changed.connect(_on_combo_changed)

	# 📱 Botón táctil para celular (cubre toda la pantalla)
	var canvas_tactil = CanvasLayer.new()
	canvas_tactil.layer = -1  # Detrás del HUD
...
func _on_combo_changed(nivel: int, multiplicador: int) -> void:
	...
	# Mostrar el combo con animación llamativa
...
func recibir_daño() -> void:
	# Si estás en dash, destruyes el obstáculo sin perder vida
	if esta_en_dash:
		return
