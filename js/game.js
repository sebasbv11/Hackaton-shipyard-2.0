/**
 * Manta: Travesía por Jocay — núcleo + 4 minijuegos (orientación horizontal)
 */
...
const LEVELS = [
  ...
  },
  {
    ...
    playerSpeed: 2.4,
    briefing:
      "Minijuego: **Ruta comercial**. Hay 3 carriles en el mar. Usa los botones ◀ ▶ (esquina inferior derecha) " +
...
const player = { x: 80, y: 0, w: 56, h: 32, speed: 2.6, invincible: 0 };
...
function startGame() {
  ...
  score = 0;
  lives = 3;
  if (window.updateOrientationUI) window.updateOrientationUI();
  showLevelBriefing();
}
...
function drawOcean(dt = 1 / 60) {
  ...
  if (lvl?.mode === "catcher") {
    ...
    ctx.fillRect(0, 0, GW(), GH() * 0.5);
  }
  if (lvl?.mode === "lanes") {
    ctx.fillStyle = "rgba(244,162,97,0.08)";
    ctx.fillRect(0, GH() - 90, GW(), 90);
  }
}
...
function drawTimer() {
  ...
  ctx.fillRect(cx - 36, 8, 72, 22);
  ctx.fillStyle = timeLeft < 15 ? "#e76f51" : "#fff";
  ctx.font = "bold 14px sans-serif";
  ctx.textAlign = "center";
  ctx.fillText(`${Math.ceil(timeLeft)}s`, cx, 24);
  ctx.textAlign = "left";
}
...
function drawModeLabel() {
  ...
  ctx.fillRect(GW() / 2 - 78, 8, 156, 20);
  ctx.fillStyle = "#f4e4bc";
  ctx.font = "600 11px sans-serif";
  ctx.textAlign = "center";
  ctx.fillText(labels[m] || "", GW() / 2, 22);
  ctx.textAlign = "left";
}
...
function render(now) {
  ...
  if (canvas.width !== GW() || canvas.height !== GH()) syncCanvasSize();
...
window.spawnSplash = spawnSplash;
