/**
 * Cuatro minijuegos distintos — uno por nivel cultural
 */
const LevelModes = {
  /** Nivel 1: Esmeraldas caen del cielo — atrapa con la balsa (solo horizontal) */
  catcher: {
    init(state) {
      state.falling = [];
      state.spawnCd = 0;
  ...
  /** Nivel 2: Pasar por canales entre arrecifes que avanzan */
  channel: {
    init(state) {
      state.walls = [];
      state.distance = 0;
    ...
    update(state, dt, lvl) {
      ...
      if (state.wallCd <= 0) {
        state.wallCd = 1.1;
        const gapH = 110 + Math.random() * 40;
        const gapY = 80 + Math.random() * (GH() - gapH - 160);
        state.walls.push({
          x: GW() + 20,
          gapY,
          gapH,
    ...
    draw(ctx, state) {
      ...
      ctx.fillRect(12, GH() - 28, GW() - 24, 10);
      ctx.fillStyle = "#e9c46a";
      ctx.fillRect(12, GH() - 28, (GW() - 24) * Math.min(1, prog), 10);
      ctx.fillStyle = "#fff";
      ctx.font = "bold 11px sans-serif";
      ctx.fillText("Puerto de Manta", GW() - 105, GH() - 34);
      drawPortIcon(ctx, GW() - 42, GH() - 52);
    },
    handleInput(state, dt, dx, dy) {
      const move = state.player.speed * GAME_PACE * dt * TARGET_FPS;
      state.player.x += dx * move;
      ...
      state.player.y = Math.max(50, Math.min(GH() - state.player.h - 50, state.player.y));
    },
    hudGoal: () => "Cruza los canales al puerto",
    progress: (state) => (state.distance / state.goalDistance) * 100,
  },

  /** Nivel 3: Tres carriles — cambia de carril y recoge conchas */
  lanes: {
    init(state) {
      state.lane = 1;
      state.laneCount = 3;
    ...
    draw(ctx, state) {
      for (let i = 0; i < 3; i++) {
        ...
        ctx.strokeRect(i * state.laneW + 4, 60, state.laneW - 8, GH() - 120);
      }
  ...
  /** Nivel 4: Red de rescate — pulsa para atrapar peces */
  rescue: {
    init(state) {
      state.swimmers = [];
      state.spawnCd = 0;
    ...
    handleInput(state, dt, dx, dy) {
      ...
      state.player.y += dy * move;
      state.player.x = Math.max(12, Math.min(GW() - state.player.w - 12, state.player.x));
      state.player.y = Math.max(70, Math.min(GH() - state.player.h - 90, state.player.y));
